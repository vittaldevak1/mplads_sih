...
def run_remaining_audit():
    try:
        ...
        # Only fetch works NOT yet in risk_scores
        print("Fetching unprocessed works...", flush=True)
        rows = db.execute(text("""
            ...
            WHERE w.work_id NOT IN (SELECT work_id FROM risk_scores)
            ORDER BY w.id
        """)).fetchall()

        total = len(rows)
        print(f"Found {total} unprocessed works (out of 88111). Starting inference...", flush=True)
        ...
        print(f"Total in DB: {db.execute(text('SELECT COUNT(DISTINCT work_id) FROM risk_scores')).scalar()}", flush=True)
