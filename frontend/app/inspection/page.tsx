  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-[#0F172A] p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl font-display text-[#0F172A]">Inspection Workflow</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          {priorityCards.map(({ key, label, count, color }) => (
            <button
              ...
              className={`rounded-xl border p-4 text-left transition ${filter === key ? 'ring-2 ring-accent' : ''}`}
              style={{ background: color.bg, borderColor: color.border }}
            >
              <div className="text-[10px] font-mono-tech uppercase tracking-wider" style={{ color: color.text }}>{label}</div>
              <div className="text-2xl font-display mt-1" style={{ color: color.text }}>{count}</div>
            </button>
          ))}
          <div className="rounded-xl border border-[#E2E8F0] bg-white p-4">
            <div className="text-[10px] font-mono-tech text-[#94A3B8] uppercase tracking-wider">Unanalyzed</div>
            <div className="text-2xl font-display text-[#64748B] mt-1">{counts.unanalyzed}</div>
          </div>
        </div>
        ...
        {notesModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
            <div className="w-full max-w-md rounded-xl border border-[#E2E8F0] bg-white p-6 shadow-2xl">
              ...
              <div className="text-[10px] font-mono-tech text-[#94A3B8] mb-3">{notesModal.workId}</div>
              <textarea
                value={notesText}
        ...
        {historyModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
            <div className="w-full max-w-lg rounded-xl border border-[#E2E8F0] bg-white p-6 shadow-2xl">
              ...
              <div className="text-[10px] font-mono-tech text-[#94A3B8] mb-3">{historyModal}</div>
              {historyLoading ? (
                ...
                <div className="space-y-3 max-h-[400px] overflow-y-auto">
                  {history.map((h) => (
                    <div key={h.id} className="p-3 rounded-lg bg-[#F8FAFC] border border-[#E2E8F0]">
                      ...
                      {h.details && (
                        <pre className="text-[10px] text-[#64748B] font-mono-tech mt-2 whitespace-pre-wrap p-2 rounded bg-white">
